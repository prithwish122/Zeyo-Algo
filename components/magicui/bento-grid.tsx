...
const BentoCard = ({ name, className, background, Icon, description, href, cta, ...props }: BentoCardProps) => (
  <div
    ...
    className={cn(
      ...
      "bg-[#18181b] [box-shadow:0_0_0_1px_rgba(255,255,255,.08),0_2px_8px_rgba(0,0,0,.18),0_12px_24px_rgba(0,0,0,.28)]",
      "border border-[#23232a] dark:bg-[#18181b] dark:border-[#23232a] dark:[box-shadow:0_-20px_80px_-20px_#ffffff0f_inset]",
      className,
    )}
    {...props}
  >
    ...
    <div className="p-4">
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-1 transition-all duration-300 lg:group-hover:-translate-y-10">
        <Icon className="h-12 w-12 origin-left transform-gpu text-[#bdbdbd] transition-all duration-300 ease-in-out group-hover:scale-75" />
        <h3 className="text-xl font-semibold text-white">{name}</h3>
        <p className="max-w-lg text-[#bdbdbd]">{description}</p>
      </div>
      ...
      >
        <Button variant="link" asChild size="sm" className="pointer-events-auto p-0 text-[#7dd3fc] hover:text-white">
          <a href={href}>
            {cta}
    ...
    <div className="pointer-events-none absolute inset-0 transform-gpu transition-all duration-300 group-hover:bg-white/[.06] group-hover:dark:bg-white/[.06]" />
  </div>
)
